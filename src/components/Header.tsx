    ...
    <header className="sticky top-0 z-50 bg-white border-b border-ehaco-border/50 shadow-sm">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-3 py-1.5 md:px-6 md:py-3.5">
        ...
        <nav className="hidden items-center gap-8 md:flex">
          <Link to="/" className="relative text-sm md:text-base font-medium text-muted hover:text-ehaco-text after:absolute after:-bottom-1 after:left-0 after:h-0.5 after:w-0 after:bg-accent after:transition-all hover:after:w-full">
            イベントを探す
          </Link>
          <Link to="/about" className="relative text-sm md:text-base font-medium text-muted hover:text-ehaco-text after:absolute after:-bottom-1 after:left-0 after:h-0.5 after:w-0 after:bg-accent after:transition-all hover:after:w-full">
            ehaco!とは
          </Link>
          <Link to="/for-organizers" className="relative text-sm md:text-base font-medium text-muted hover:text-ehaco-text after:absolute after:-bottom-1 after:left-0 after:h-0.5 after:w-0 after:bg-accent after:transition-all hover:after:w-full">
            イベント掲載
          </Link>
        </nav>
            ...
            {notifOpen && (
              <div className="absolute right-0 top-full mt-3 w-96 overflow-hidden rounded-2xl border border-ehaco-border bg-white shadow-2xl hidden md:block">
                  ...
                  {notifData.slice(0, 5).map((notif) => {
                    ...
                    return (
                      <Link
                        ...
                        className={`flex gap-3 border-b border-ehaco-border/50 px-5 py-4 transition last:border-b-0 hover:bg-gray-50 ${!isRead ? 'bg-accent/5' : ''}`}
                      >
                ...
                <div className="border-t border-ehaco-border px-5 py-3 text-center">
                  <Link to="/mypage/notifications" className="text-sm font-medium text-accent transition hover:text-accent-light" onClick={() => setNotifOpen(false)}>
                    すべてのお知らせを見る
                  </Link>
                </div>
              </div>
            ...
            {userMenuOpen && (
              <div className="absolute right-0 top-full mt-3 w-56 overflow-hidden rounded-2xl border border-ehaco-border bg-white py-2 shadow-2xl">
                {[
                  ...
                  <Link key={to} to={to} className="flex items-center gap-3 px-4 py-2.5 text-base text-ehaco-text transition hover:bg-gray-50" onClick={() => setUserMenuOpen(false)}>
                ...
                <Link to="/org/dashboard" className="flex items-center gap-3 px-4 py-2.5 text-base text-accent transition hover:bg-gray-50" onClick={() => setUserMenuOpen(false)}>
      ...
      <div className={`overflow-hidden md:hidden transition-all duration-300 ease-in-out ${
        mobileMenuOpen ? 'max-h-[500px] opacity-100 bg-white border-t border-ehaco-border/50' : 'max-h-0 opacity-0'
      }`}>
        <nav className="space-y-1 px-4 py-3">
          <Link to="/" className="block rounded-xl px-3 py-2.5 text-base font-medium text-ehaco-text hover:bg-gray-50 transition" onClick={() => setMobileMenuOpen(false)}>
            イベントを探す
          </Link>
          <Link to="/for-organizers" className="block rounded-xl px-3 py-2.5 text-base font-medium text-ehaco-text hover:bg-gray-50 transition" onClick={() => setMobileMenuOpen(false)}>
            イベント掲載
          </Link>
        </nav>
        ...
        <div className="px-4 py-3">
          ...
          <nav className="space-y-1">
            {['ダッシュボード', '申込済みイベント', 'お気に入り', 'フォロー中', 'お知らせ', 'プロフィール', '設定'].map((label, i) => {
              ...
              return (
                <Link key={label} to={paths[i]} className="block rounded-xl px-3 py-2.5 text-sm text-ehaco-text hover:bg-gray-50 transition" onClick={() => setMobileMenuOpen(false)}>
                  {label}
                </Link>
              );
            })}
        ...
        <div className="px-4 py-3">
          <Link to="/login" className="block rounded-xl px-3 py-2.5 text-sm text-red-500 hover:bg-gray-50 transition" onClick={() => setMobileMenuOpen(false)}>
            ログアウト
          </Link>
        </div>
      </div>
