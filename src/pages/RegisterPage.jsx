...
const stepLabels = ['アカウント', 'お名前', '会社情報', '企業属性', '同意'];
...
export default function RegisterPage() {
  ...
  const [detailStep, setDetailStep] = useState(0); // 0=account(done), 1=name, 2=company, 3=attributes, 4=agree
  ...
  const back = () => { if (detailStep > 1) { setDetailStep(detailStep - 1); window.scrollTo(0, 0); } };
    ...
    return (
      <div className="min-h-screen bg-ehaco-bg flex items-center justify-center px-4 fade-in">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={(e) => { e.preventDefault(); setFlow('verify'); }} className="bg-white rounded-2xl ring-1 ring-ehaco-border/50 p-6 space-y-4">
            <div>
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} required minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} className={`${inputClass} pr-12`} placeholder="8文字以上" />
      ...
      <div className="max-w-xl mx-auto px-4 sm:px-6 py-8">
        ...
        {detailStep === 2 && (
          <div className="bg-white rounded-2xl ring-1 ring-ehaco-border/50 p-6 md:p-8">
            ...
            <div className="space-y-4">
              ...
              <div><label className={labelClass}>会社URL {req}</label><input type="url" value={form.companyUrl} onChange={(e) => update('companyUrl', e.target.value)} className={inputClass} placeholder="https://example.com" /></div>
              <div>
                <label className={labelClass}>電話番号 {req}</label>
                <input type="tel" value={form.phone} onChange={(e) => update('phone', e.target.value)} className={inputClass} placeholder="直通番号（例：09012345678）" />
              </div>
              <div>
                <label className={labelClass}>所在地 {req}</label>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-sm text-muted">〒</span>
                  <input type="text" value={form.zip1} onChange={(e) => update('zip1', e.target.value)} className={`${inputClass} w-28`} placeholder="100" maxLength={3} />
                  <span className="text-muted">ー</span>
                  <input type="text" value={form.zip2} onChange={(e) => update('zip2', e.target.value)} className={`${inputClass} w-32`} placeholder="0001" maxLength={4} />
                </div>
        ...
        {detailStep === 3 && (
          <div className="bg-white rounded-2xl ring-1 ring-ehaco-border/50 p-6 md:p-8">
            ...
            <div className="space-y-4">
              <div><label className={labelClass}>業種 {req}</label><select value={form.industry} onChange={(e) => update('industry', e.target.value)} className={selectClass}><option value="">選択してください</option>{industries.map((v) => <option key={v}>{v}</option>)}</select></div>
              <div><label className={labelClass}>ビジネス形態 {req}</label><select value={form.bizType} onChange={(e) => update('bizType', e.target.value)} className={selectClass}><option value="">選択してください</option>{bizTypes.map((v) => <option key={v}>{v}</option>)}</select></div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div><label className={labelClass}>従業員規模 {req}</label><select value={form.employees} onChange={(e) => update('employees', e.target.value)} className={selectClass}><option value="">選択</option>{employeeSizes.map((v) => <option key={v}>{v}</option>)}</select></div>
                <div><label className={labelClass}>年商規模 {req}</label><select value={form.revenue} onChange={(e) => update('revenue', e.target.value)} className={selectClass}><option value="">選択</option>{revenueSizes.map((v) => <option key={v}>{v}</option>)}</select></div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div><label className={labelClass}>職種 {req}</label><select value={form.department} onChange={(e) => update('department', e.target.value)} className={selectClass}><option value="">選択</option>{departments.map((v) => <option key={v}>{v}</option>)}</select></div>
                <div><label className={labelClass}>役職 {req}</label><select value={form.position} onChange={(e) => update('position', e.target.value)} className={selectClass}><option value="">選択</option>{positions.map((v) => <option key={v}>{v}</option>)}</select></div>
              </div>
            </div>
          </div>
        )}
