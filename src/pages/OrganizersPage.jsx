            ...
            return (
              <Link key={org.id} to={`/organizer/${org.id}`} className="block bg-white rounded-2xl ring-1 ring-ehaco-border/50 p-5 hover:shadow-lg hover:ring-accent/20 transition-all">
                ...
                <div className="mt-3 pt-3 border-t border-ehaco-border/50">
                  ...
                  <span onClick={(e) => { e.preventDefault(); e.stopPropagation(); window.location.hash = `/event/${org.latestEvent.id}`; }} className="text-sm text-ehaco-text hover:text-accent transition line-clamp-1 font-medium cursor-pointer">
                ...
                {/* Follow button */}
                <button
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleFollow(org.id); }}
