...
export default function OrgNotificationsPage() {
  const [readState, setReadState] = useState<Map<number, boolean>>(() => new Map(notifications.map((n) => [n.id, n.read])));
  const [filter, setFilter] = useState<string>('all');

  const unreadCount = [...readState.values()].filter((v) => !v).length;
  const markAsRead = (id: number) => setReadState((prev) => { const next = new Map(prev); next.set(id, true); return next; });
