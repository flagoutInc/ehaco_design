        ...
        <div className="max-w-3xl">
          <div className="bg-white rounded-2xl ring-1 ring-ehaco-border/50 p-6 md:p-8 space-y-5">
            ...
            <div>
              ...
              <input type="text" value={subject} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSubject(e.target.value)} className={inputClass} placeholder="例）開催日時変更のお知らせ" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-ehaco-text mb-1.5">本文</label>
              <textarea value={body} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setBody(e.target.value)} className={`${inputClass} min-h-[200px] resize-none`} placeholder="参加者に伝えたい内容を入力" />
            </div>
