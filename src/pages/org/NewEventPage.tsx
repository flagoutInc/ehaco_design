...
function Progress({ current }: ProgressProps) {
  return (
...
export default function NewEventPage() {
  ...
  const set = (key: keyof EventForm, value: EventForm[keyof EventForm]) => setForm((prev) => ({ ...prev, [key]: value }));
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => { const f = e.target.files?.[0]; if (f) { set('eventImage', f); set('eventImagePreview', URL.createObjectURL(f)); } };
  const addTicket = () => setForm((prev) => ({ ...prev, tickets: [...prev.tickets, { name: '', quantity: '' }] }));
  const removeTicket = (i: number) => setForm((prev) => ({ ...prev, tickets: prev.tickets.filter((_, idx) => idx !== i) }));
  const updateTicket = (i: number, field: keyof Ticket, value: string) => { setForm((prev) => { const t = [...prev.tickets]; t[i] = { ...t[i], [field]: value }; return { ...prev, tickets: t }; }); };
