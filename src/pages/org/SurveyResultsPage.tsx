...
interface QuestionResult {
  question: string;
  type: string;
  answers: (RadioAnswer | TextAnswer)[];
}

const mockResults: { totalResponses: number; responseRate: number; questions: QuestionResult[] } = {
  totalResponses: 89,
  responseRate: 72,
  questions: [
    {
        ...
        {mockResults.questions.map((q, qi) => (
          <div key={qi} className="bg-white rounded-2xl ring-1 ring-ehaco-border/50 p-6">
            ...
            {q.type !== 'text' ? (
                  ...
                  return (
                    <div key={ai}>
                      <div className="flex items-center justify-between text-sm mb-1">
                        <span className="text-ehaco-text">{ra.label}</span>
                        <span className="text-muted">{ra.count}件 ({ra.pct}%)</span>
                      </div>
              ...
              <div className="space-y-3">
                {q.answers.map((a, ai) => {
                  const ta = a as TextAnswer;
                  return (
                    <div key={ai} className="bg-gray-50 rounded-lg p-4">
                      <p className="text-sm text-ehaco-text">{ta.text}</p>
                      <p className="text-xs text-muted mt-2">{ta.date}</p>
                    </div>
                  );
                })}
