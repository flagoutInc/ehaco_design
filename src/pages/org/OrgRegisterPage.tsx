  ...
  return (
    <div className="min-h-screen bg-ehaco-bg fade-in">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        ...
        <form onSubmit={(e: React.FormEvent<HTMLFormElement>) => { e.preventDefault(); setStep('complete'); }} className="space-y-6">
          <section className="bg-white rounded-2xl ring-1 ring-ehaco-border/50 p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" required value={form.contactName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => update('contactName', e.target.value)}
                  className={inputClass} placeholder="例：田中 太郎" />
              </div>
              <div>
                <label className={labelClass}>メールアドレス {req}</label>
                <input type="email" required value={form.email} onChange={(e: React.ChangeEvent<HTMLInputElement>) => update('email', e.target.value)}
                  className={inputClass} placeholder="会社メールアドレス" />
              </div>
              <div>
                <label className={labelClass}>パスワード {req}</label>
                <input type="password" required minLength={8} value={form.password} onChange={(e: React.ChangeEvent<HTMLInputElement>) => update('password', e.target.value)}
                  className={inputClass} placeholder="半角英数字8文字以上" />
              </div>
              <div>
                <label className={labelClass}>電話番号 {req}</label>
                <input type="tel" required value={form.phone} onChange={(e: React.ChangeEvent<HTMLInputElement>) => update('phone', e.target.value)}
          ...
          <section className="bg-white rounded-2xl ring-1 ring-ehaco-border/50 p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" required value={form.companyName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => update('companyName', e.target.value)}
                  className={inputClass} placeholder="例：株式会社〇〇" />
              </div>
              <div>
                <label className={labelClass}>会社URL</label>
                <input type="url" value={form.website} onChange={(e: React.ChangeEvent<HTMLInputElement>) => update('website', e.target.value)}
                  className={inputClass} placeholder="https://example.com" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className={labelClass}>業種 {req}</label>
                  <select required value={form.industry} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => update('industry', e.target.value)} className={selectClass}>
                ...
                <div>
                  ...
                  <select required value={form.employees} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => update('employees', e.target.value)} className={selectClass}>
                    <option value="">選択してください</option>
