        ...
        <div className="bg-white rounded-2xl shadow-sm ring-1 ring-ehaco-border/50 p-6 md:p-8 space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" required className={inputClass} value={form.lastName}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => set('lastName', e.target.value)} placeholder="例：田中" />
            </div>
            <div>
              ...
              <input type="text" required className={inputClass} value={form.firstName}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => set('firstName', e.target.value)} placeholder="例：太郎" />
            </div>
          </div>
          ...
          <div>
            ...
            <input type="email" required className={inputClass} value={form.email}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => set('email', e.target.value)} placeholder="例：tanaka@example.com" />
          </div>
          ...
          <div>
            ...
            <input type="tel" required className={inputClass} value={form.phone}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => set('phone', e.target.value)} placeholder="直通番号（例：08012345678）" />
          </div>
        </div>
