...
type TabLabel = 'すべて' | '開催予定' | '開催済み';

const tabs: TabLabel[] = ['すべて', '開催予定', '開催済み'];

const myEvents = events.slice(0, 5).map((event, i) => ({
  ...event,
  status: (i < 3 ? '開催予定' : '開催済み') as '開催予定' | '開催済み',
}));

export default function MyEventsPage() {
  const [activeTab, setActiveTab] = useState<TabLabel>('すべて');
  ...
  const tabCount = (tab: TabLabel): number => tab === 'すべて' ? myEvents.length : tab === '開催予定' ? upcomingCount : pastCount;
    ...
    <div className="fade-in">
      ...
      {filtered.length > 0 ? (
        ...
        <EmptyState
          ...
          title="申込済みイベントはありません"
          subtitle="気になるイベントを見つけて参加しましょう"
