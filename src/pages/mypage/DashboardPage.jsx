      ...
      <div className="mt-6">
        ...
        <div className="grid grid-cols-3 gap-3 md:gap-4 mb-8">
          {stats.map((stat) => (
            <button
              ...
              className={`rounded-2xl p-4 md:p-5 text-left transition-all ${stat.color} ${
                expandedSection === stat.key ? 'ring-2 ring-current shadow-md' : 'hover:shadow-md'
              }`}
            >
        ...
        {expandedSection === 'events' && (
          <div className="mb-8 animate-[fadeIn_0.2s_ease-out]">
            <h2 className="text-lg font-bold text-ehaco-text mb-4">申込済みイベント</h2>
        ...
        {expandedSection === 'favorites' && (
          <div className="mb-8 animate-[fadeIn_0.2s_ease-out]">
            <h2 className="text-lg font-bold text-ehaco-text mb-4">お気に入り</h2>
            <div className="space-y-4">
        ...
        {expandedSection === 'following' && (
          <div className="mb-8 animate-[fadeIn_0.2s_ease-out]">
            <h2 className="text-lg font-bold text-ehaco-text mb-4">フォロー中の新着</h2>
            <div className="space-y-4">
              {followUpdates.map((item, i) => (
                <div key={i} className="bg-white rounded-2xl ring-1 ring-ehaco-border/50 p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <img src={item.logo} alt={item.organizer} className="w-8 h-8 rounded-full shrink-0" />
                  ...
                  >
                    <img src={item.event.image} alt={item.event.title} className="w-16 h-12 rounded-lg object-cover shrink-0" />
                    <div className="min-w-0">
        ...
        {!expandedSection && (
          <>
            {/* 次のイベント */}
            <div className="mb-8">
              <h2 className="text-lg font-bold text-ehaco-text mb-4">次のイベント</h2>
              {registeredEvents[0] ? (
                <Link
                ...
                >
                  <div className="flex gap-5">
                    <img
                      ...
                      className="w-[140px] md:w-[200px] rounded-xl object-cover shrink-0"
                    />
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                      <p className="text-sm text-muted mb-1">{registeredEvents[0].date}</p>
                      <h3 className="text-lg font-bold text-ehaco-text mb-2 line-clamp-2">{registeredEvents[0].title}</h3>
            ...
            <div>
              ...
              <div className="space-y-3">
                {registeredEvents.map((event) => (
                  ...
                  >
                    <img src={event.image} alt={event.title} className="w-[120px] h-[90px] object-cover shrink-0" />
