...
export default function NotificationsPage() {
  const [readState, setReadState] = useState<Map<number, boolean>>(() => new Map(notifData.map((n) => [n.id, n.read])));
  const [filter, setFilter] = useState<'all' | 'unread'>('all');
  const [page, setPage] = useState<number>(1);

  const unreadCount = [...readState.values()].filter((v) => !v).length;
  const markAsRead = (id: number): void => setReadState((prev) => { const next = new Map(prev); next.set(id, true); return next; });
  const getNotifLink = (notif: typeof notifData[number]): string => `/mypage/notifications/${notif.id}`;
  const markAllAsRead = (): void => setReadState((prev) => { const next = new Map(prev); for (const k of next.keys()) next.set(k, true); return next; });

  const filtered = filter === 'unread' ? notifData.filter((n) => !readState.get(n.id)) : notifData;
  const totalPages = Math.ceil(filtered.length / PER_PAGE);
  const paged = filtered.slice((page - 1) * PER_PAGE, page * PER_PAGE);

  const changePage = (p: number): void => { setPage(p); window.scrollTo(0, 0); };
  const changeFilter = (f: 'all' | 'unread'): void => { setFilter(f); setPage(1); };
  ...
  return (
    <div className="fade-in">
      ...
      <div className="flex gap-2 mb-6">
        {([{ key: 'all' as const, label: 'すべて' }, { key: 'unread' as const, label: '未読のみ' }]).map((t) => (
